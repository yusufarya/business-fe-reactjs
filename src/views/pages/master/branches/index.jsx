...
const Index = () => {
    ...
    const [branchData, setUnitData] = useState(null)
    ...
    const handleDeleteClick = (branchId, branchName) => {
        setVisible(!visible)
        setRowData({id:branchId, name:branchName})
        setAction('delete')
    }
    ...
    return (
        <div>
                        ...
                        <CTableBody>
                        {
                            ...
                            branchData.map((branch, index) => (
                                <CTableRow key={branch.id}>
                                    ...
                                    <CTableDataCell>{branch.description}</CTableDataCell>
                                    <CTableDataCell style={{textAlign:'center'}}>
                                        <CButton color="warning" size="sm" onClick={() => handleEditClick(branch.id)}>
                                            <CIcon style={{color:'azure'}} icon={cilPencil}/>
                                        </CButton>
                                        &nbsp;&nbsp;
                                        <CButton color="danger" size="sm" onClick={() => handleDeleteClick(branch.id, branch.name)}>
            ...
            {action == 'edit' &&
                (visible && <ModalEditData status={visible} onModalStatusChange={handleModalStatusChange} branchId={rowData} successUpdate={successUpdate}/>)
            }

            {action == 'delete' &&
                (visible && <ModalDelete status={visible} onModalStatusChange={handleModalStatusChange} paramsBranch={rowData} successUpdate={successUpdate} />)
            }
