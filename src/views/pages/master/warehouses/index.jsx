...
const Index = () => {
    ...
    useEffect(() => {
        const getDataUnit = async () => {
            try {
                const result = await AppService.serviceGet('api/all-warehouse')
    ...
    const handleEditClick = (warehouseId) => {
        setVisible(!visible)
        setRowData(warehouseId)
        setAction('edit')
    }
    const handleDeleteClick = (warehouseId, warehouseName) => {
        setVisible(!visible)
        setRowData({id:warehouseId, name:warehouseName})
        setAction('delete')
    }
    ...
    return (
        <div>
                        ...
                        <CTableBody>
                        {
                            ...
                            warehouseData.map((warehouse, index) => (
                                <CTableRow key={warehouse.id}>
                                    ...
                                    <CTableDataCell>{warehouse.address}</CTableDataCell>
                                    <CTableDataCell>{warehouse.description}</CTableDataCell>
                                    <CTableDataCell style={{textAlign:'center'}}>
                                        <CButton color="warning" size="sm" onClick={() => handleEditClick(warehouse.id)}>
                                            <CIcon style={{color:'azure'}} icon={cilPencil}/>
                                        </CButton>
                                        &nbsp;&nbsp;
                                        <CButton color="danger" size="sm" onClick={() => handleDeleteClick(warehouse.id, warehouse.name)}>
            ...
            {action == 'edit' &&
                (visible && <ModalEditData status={visible} onModalStatusChange={handleModalStatusChange} warehouseId={rowData} successUpdate={successUpdate}/>)
            }
